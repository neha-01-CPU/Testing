/* ================================================================
   PICAZO — script.js  v4.2 (Fixed Timer, Restored Popups & Bots)
================================================================ */
'use strict';
...
const WORD_BANK = [
  ...
  {w:'lighthouse',e:'🏮'},{w:'hurricane',e:'🌀'},{w:'keyboard',e:'⌨️'}
];
...
const screenLobby = $('screen-lobby'), screenGame = $('screen-game'), btnAvPrev = $('btn-av-prev'), btnAvNext = $('btn-av-next');
const avCanvas = $('av-canvas'), avFrame = $('av-frame'), avDots = $('av-dots'), inpName = $('inp-name'), btnPlay = $('btn-play'), btnPrivate = $('btn-private');
const timerNum = $('timer-num'), tFg = $('t-fg'), roundBadge = $('round-badge'), wordDisplay = $('word-display'), wordMeta = $('word-meta');
const btnMute = $('btn-mute'), muteIcon = $('mute-icon'), playerList = $('player-list'), chatMessages = $('chat-messages');
const chatInput = $('chat-input'), btnChatSend = $('btn-chat-send'), gameCanvas = $('game-canvas'), canvasWrap = $('canvas-wrap');
const ctx = gameCanvas.getContext('2d', { willReadFrequently: true });
const overlayWaiting = $('overlay-waiting'), overlayWordSelect = $('overlay-word-select'), overlayRoundEnd = $('overlay-round-end');
const wsClock = $('ws-timer'), wsTimerBar = $('ws-timer-bar'), wsCards = $('ws-cards'), btnCopyLink = $('btn-copy-link');
const reEmoji = $('re-emoji'), reTitle = $('re-title'), reWordVal = $('re-word-val'), reScores = $('re-scores'), reCountdown = $('re-countdown'), reNext = $('re-next');
const eventPopup = $('event-popup'), eventPopupIcon = $('event-popup-icon'), eventPopupMsg = $('event-popup-msg');
  ...
  startRound: function() {
    ...
    const guessInt = setInterval(() => {
      ...
      if (rand < 0.15 && S.currentWord) {
        ...
        const gibberish = ["tree", "house", "car", "dog?", "sun", "cloud"];
...
function drawAvatar(canvas, def, size = 96) {
  const c = canvas.getContext('2d'), W = size, H = size;
  c.clearRect(0, 0, W, H);
  const bg = c.createLinearGradient(0, 0, W, H);
  bg.addColorStop(0, def.accent + '44'); bg.addColorStop(1, def.accent + '18');
...
function setAvatar(i) {
  ...
  AVATAR_DEFS.forEach((_, j) => {
    ...
    d.addEventListener('click', () => setAvatar(j)); avDots.appendChild(d);
  });
...
btnPlay.addEventListener('click', () => {
  ...
  if (!audioCtx) audioCtx = new (window.AudioContext || window.webkitAudioContext)();
  S.playerName = name; S.totalRounds = 3; S.drawTime = 45; S.maxPlayers = 8; S.hintsCount = 2;
...
function initGame() {
  ...
  setupToolbar(); setupChat(); setupMuteBtn(); initCanvas();
  ...
  buildLeaderboard();
  setTimeout(() => { overlayWaiting.classList.add('hidden'); showEventPopup('🎮', 'Game started!'); startWordSelection(); }, 2000);
}
...
function updateTimerUI() {
  ...
  // FIX: SVGs require setAttribute, not className
...
function startWordSelection() {
  ...
  let t = 15; wsClock.textContent = t; wsTimerBar.style.transition = 'none'; wsTimerBar.style.width = '100%';
  clearInterval(S.wsTimerInterval);
  S.wsTimerInterval = setInterval(() => {
    t--; wsClock.textContent = t; wsTimerBar.style.transition = 'width 1s linear'; wsTimerBar.style.width = (t/15*100)+'%';
...
function renderWordBlanks() {
  wordDisplay.innerHTML = ''; if (!S.currentWord) return;
...
function endRound(allGuessed = false) {
  ...
  if (S.guessedIds.size > 0 && S.players[S.drawerIdx]) S.players[S.drawerIdx].score += Math.min(S.guessedIds.size * 30, 150);
  const sorted = [...S.players].sort((a, b) => b.score - a.score);
  reEmoji.textContent = allGuessed ? '🎉' : '⏰'; reTitle.textContent = allGuessed ? 'Everyone guessed!' : 'Round Over!'; reWordVal.textContent = S.currentWord;
  reScores.innerHTML = sorted.map((p, i) => `<div class="re-score-row" style="animation-delay:${i*0.07}s"><span class="re-score-name">${i===0?'🥇':i===1?'🥈':i===2?'🥉':''} ${escHtml(p.name)}</span><span class="re-score-pts">${p.score} pts</span></div>`).join('');
  overlayRoundEnd.classList.remove('hidden');
  let cd = 5; reCountdown.textContent = cd;
  const cdInt = setInterval(() => { cd--; reCountdown.textContent = cd; if (cd <= 0) { clearInterval(cdInt); overlayRoundEnd.classList.add('hidden'); nextRound(); } }, 1000);
}
...
function endGame() {
  ...
  overlayRoundEnd.classList.remove('hidden'); reEmoji.textContent = '🏆'; reTitle.textContent = 'Game Over!'; reWordVal.textContent = winner.name + ' wins!'; reNext.style.display = 'none';
...
function onPointerDown(e) { if (!S.isDrawer) return; gameCanvas.setPointerCapture(e.pointerId); S.isDrawing = true; const pos = getPointerXY(e); ctx.beginPath(); ctx.moveTo(pos.x, pos.y); ctx.strokeStyle = S.tool === 'eraser' ? '#ffffff' : S.color; ctx.lineWidth = S.brushSize; }
...
function setupToolbar() {
  ['pencil','eraser'].forEach(t => { $('tool-' + t).addEventListener('click', () => { S.tool = t; gameCanvas.className = t === 'eraser' ? 'eraser' : ''; }); });
  ...
  $('color-palette').innerHTML = COLORS.map(hex => `<div class="c-swatch ${hex===S.color?'active':''}" style="background:${hex}" onclick="S.color='${hex}'; document.getElementById('color-indicator').style.background='${hex}'; if(S.tool==='eraser'){S.tool='pencil'; gameCanvas.className='';}"></div>`).join('');
}

/* ════════════════════════════════════════════
   CHAT & POPUPS (Fully Restored)
════════════════════════════════════════════ */
function setupChat() {
...
function sendGuess() {
  ...
  if (word && guess === word) {
    ...
    showEventPopup('🎉', `${S.playerName} guessed it!`);
...
function setupMuteBtn() {
  btnMute.addEventListener('click', () => {
    S.isMuted = !S.isMuted;
    muteIcon.innerHTML = S.isMuted ? `<polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/>` : `<polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07"/>`;
  });
}

function showEventPopup(icon, msg) {
  eventPopupIcon.textContent = icon; eventPopupMsg.textContent = msg;
  eventPopup.classList.remove('hidden'); setTimeout(() => eventPopup.classList.add('hidden'), 2800);
}
